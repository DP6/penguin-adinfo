    ...
    _buildGeneralParams() {
        Object.keys(this.config.medias[this._vehicleName]).forEach((param) => {
            ...
            csvColumns.forEach((column) => {
                ...
                const validateColumnExistsError = new ValidateColumnExistsHandler_1.ValidateColumnExistsHandler(this.config, column);
                ...
                if (this.csvLine[normalizedColumn]) {
                    this._params[param] += `${StringUtils_1.StringUtils.replaceWhiteSpace(this.csvLine[normalizedColumn], this.config.spaceSeparator).toLowerCase()}${this.config.separator}`;
                }
            });
