...
class UserDAO {
        ...
        return this._objectStore
            ...
            if (allAdvertiserUsers.length > 0) {
                allAdvertiserUsers.forEach((advertiserUser) => {
                    if (advertiserUser.permission !== 'owner' ||
                        (userRequestPermission === 'admin' && advertiserUser.permission === 'user')) {
                        const user = new User_1.User(advertiserUser.userid, advertiserUser.permission, advertiserUser.advertiser, advertiserUser.email, advertiserUser.active, advertiserUser.adOpsTeam);
                        users.push(user);
                    }
                });
            }
    ...
    getAllUsersFromAdOpsTeam(advertiserId, adOpsTeamId) {
        ...
        return this._objectStore
            ...
            usersDocuments.docs.map((userDocument) => {
                users.push(new User_1.User(userDocument.get('id'), userDocument.get('permission'), userDocument.get('advertiser'), userDocument.get('email'), userDocument.get('active'), userDocument.get('adOpsTeam')));
            });
            return users;
        })
            .catch((err) => {
    ...
    getUser() {
        return this._objectStore
            ...
            if (allUsersDocuments.length > 0) {
                ...
                const user = new User_1.User(userToValidate.userid, userToValidate.permission, userToValidate.advertiser, userToValidate.email, userToValidate.active, userToValidate.adOpsTeam);
                return user;
            }
            else {
                throw new Error('Email ou senha incorreto(s)!');
    ...
    deactivateUser(userId, userRequestPermission) {
        return this._objectStore
            ...
            if (userToDeactivate.permission === 'user' ||
    ...
    reactivateUser(userId, userRequestPermission) {
        return this._objectStore
            ...
            if (userToReactivate.permission === 'user' ||
