...
class GoogleAds extends Vehicle_1.Vehicle {
    ...
    buildedLine() {
        const hasError = Object.keys(this._adsParams).filter((adsParam) => this._hasValidationError[adsParam] || this._hasUndefinedParameterError[adsParam]);
        return {
    ...
    _buildAdsParams() {
        Object.keys(this.config.medias.googleads).forEach((googleAdsParam) => {
            ...
            fields.forEach((column) => {
                ...
                const validateRulesForColumnHandler = new ValidateRulesForColumnHandler_1.ValidateRulesForColumnHandler(this.config, column);
                ...
                try {
                    ...
                    this._adsParams[googleAdsParam] += `${StringUtils_1.StringUtils.replaceWhiteSpace(this.csvLine[columnNormalized], this.config.spaceSeparator).toLowerCase()}${this.config.separator}`;
                }
                catch (e) {
