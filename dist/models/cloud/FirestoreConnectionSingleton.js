...
class FirestoreConnectionSingleton extends ObjectStore_1.ObjectStore {
    ...
    getDocumentFiltered(collection, conditions) {
        ...
        for (let i = 0; i < conditions.length; i++) {
            if (i == 0)
                query = collection.where(conditions[i].key, conditions[i].operator, conditions[i].value);
            else
                query = query.where(conditions[i].key, conditions[i].operator, conditions[i].value);
        }
        return query.get();
    }
}
