...
class GA extends AnalyticsTool_1.AnalyticsTool {
    ...
    _hasErrorAtUtm(utm) {
        return (this._hasUndefinedParameterError[utm] ||
            this._hasValidationError[utm]);
    }
    _errorMessageAtUtm(utm) {
        const errorMessages = [];
        if (this._hasUndefinedParameterError[utm]) {
            errorMessages.push(this._undefinedParameterErroMessage[utm].slice(0, -1));
        }
    ...
    _buildUtms() {
        Object.keys(this.config).forEach((utm) => {
            ...
            this._utms[utm] = StringUtils_1.StringUtils.replaceWhiteSpace(StringUtils_1.StringUtils.normalize(utmString).slice(0, -1), this.spaceSeparator);
        });
    }
    _buildUrl() {
        let utmString = '';
